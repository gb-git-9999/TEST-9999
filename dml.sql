...
/*INSERT INTO 테이블명
 * VALUES 값1, 값2, 값3, 값4
 * SEQ_테이블명.NEXTVAL은 오라클 데이터베이스에서 특정 시퀀스 객체의 다음 값을 가져오는 기능을 제공 */
...
FROM TBL_OWNER JOIN TBL_PET
ON TBL_OWNER.ID = TBL_PET.OWNER_ID 
...
SELECT P.ID, P.PET_ILL_NAME, P.PET_NAME, P.PET_AGE, P.WEIGHT 
FROM TBL_OWNER O JOIN TBL_PET P
...
WHERE P.PET_ILL_NAME NOT LIKE '%염%';
/**********************************************************************/
 ...
 * FROM절: 인라인뷰
 * FROM절에서의 서브쿼리 (인라인 뷰)
 * 서브쿼리가 FROM 절에 사용될 때, 이를 "인라인 뷰"라고 합니다.
 * 인라인 뷰는 테이블처럼 동작하여, 서브쿼리의 결과를 테이블처럼 사용할 수 있게 해줍니다.
 * 이를 통해 복잡한 쿼리를 간결하게 작성하거나,
 * 서브쿼리의 결과를 메인 쿼리에서 다시 사용할 수 있습니다.: 서브쿼리
 * 
 * WHERE절
 * WHERE절에서의 서브쿼리
 * WHERE 절에서 사용되는 서브쿼리는 주로 특정 조건을 만족하는 데이터를 필터링하는데 사용된다.
 * 이러한 서브쿼리는 주로 비교 연산자와 함께 사용되며,
 * 조건에 맞는 값을 찾거나, 특정 기준을 만족하는지 여부를 체크합니다.
 * 
 * SELECT절: 스칼라 서브쿼리
 * SELECT절에서의 서브쿼리 (스칼라 서브쿼리)
 * SELECT절에서의 서브쿼리는 각 행에 대해 하나의 값을 반환하는 "스칼라 서브쿼리"로 사용된다.
 * 이는 메인 쿼리의 결과에 추가적인 정보를 계산하거나 표시할 때 사용됩니다.
 * 
 * 서브쿼리 사용 시 고려 사항
 * 성능: 서브쿼리는 각 행마다 별도로 실행될 수 있으므로, 성능에 영향을 미칠 수 있습니다.
 * 특히 큰 테이블에서 사용되는 경우, 실행 계획을 최적화하는 것이 중요합니다.
 * 단일 값 반환: WHERE 절이나 SELECT 절에서의 서브쿼리는 보통 단일 값을 반환해야 합니다. 그렇지 않으면 오류가 발생할 수 있습니다. 다만, IN 연산자와 함께 사용되는 경우 다중 값을 반환할 수 있습니다.
 * 스칼라 서브쿼리의 제한: 스칼라 서브쿼리는 각 행에 대해 하나의 값을 반환해야 하며,
 * 여러 개의 값을 반환할 수 없습니다. 그렇지 않으면 오류가 발생합니다.
 * 
 * */
